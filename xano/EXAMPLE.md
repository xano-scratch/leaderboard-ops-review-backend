# Building your leaderboard-ops-review-backend backend
...
6. **Compile** with `npm run xano:export`, and **deploy** with
   ...
   "`xano.lock` — commit it" in the project README. The export also writes
   `xanoscript/`, the backend rendered as XanoScript — commit that too, never edit it.
